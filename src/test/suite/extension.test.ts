...
suite("Connections to an IRIS container", () => {
  ...
  for (const folder of CLIENT_SIDE) {
    ...
    // never goes inactive. Skipped until that is fixed.
    const toggleTest = folder === "client-compose" ? test.skip : test;
    toggleTest(`${folder}: turning objectscript.conn.active off and on is honoured`, async () => {
  ...
  for (const folder of CLIENT_SIDE) {
    ...
    test.skip(`${folder}: a connection check after the session expired recovers without prompting`, async () => {
